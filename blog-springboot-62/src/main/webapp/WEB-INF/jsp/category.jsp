<%@page import="com.yc.blog.utils.Util"%>
    ...
    <div class="content">
      ...
      <c:forEach items="${articleList }" var="a">
	      <article class="excerpt excerpt-1"><a class="focus" href="article?id=${a.id }" title=""><img class="thumb" data-original="images/excerpt.jpg" src="images/excerpt.jpg" alt=""></a>
	        <header><a class="cat" href="program">${a.categoryid }<i></i></a>
	          <h2><a href="article?id=${a.id }" title="">${a.title }</a></h2>
	        </header>
	        <p class="meta">
	          <time class="time"><i class="glyphicon glyphicon-time"></i> ${a.createtime }</time>
	          <span class="views"><i class="glyphicon glyphicon-eye-open"></i> 共${a.readcnt }人围观</span> <a class="comment" href="article?id=${a.id }#comment"><i class="glyphicon glyphicon-comment"></i> 0个不明物体</a></p>
	          <% Article a = (Article) pageContext.getAttribute("a"); %>
	        <p class="note"><%=Util.subTag(a.getContent()).substring(0, 100) %>...</p>
	      </article>
      </c:forEach>
